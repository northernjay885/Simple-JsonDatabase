...
class TransactionExample {
    ...
    public static long calcSumOfCanceledTransOnNonEmptyAccounts(List<Account> accounts) {
        ...
        return accounts.stream().filter(a -> a.getBalance() > 0).flatMap(a -> a.getTransactions().stream()).filter(t -> t.getState() == State.CANCELED).map(Transaction::getSum).reduce(0L, Long::sum);
    }
